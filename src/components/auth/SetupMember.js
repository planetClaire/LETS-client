...
import { LETS_GROUP, TITLES, LABELS, MESSAGES } from '../../Constants';
...
export default function SetupMember() {
	const submittedMessage = MESSAGES.SETUP_SUBMITTED;
	...
	if (!memberData.member) {
		return (
			<>
				<Header title={TITLES.SETUP_YOUR_MEMBERSHIP}></Header>
				<Main id="member-setup">
	...
	if (!memberData.member.approved) {
		return (
			<>
				<Header title={TITLES.SETUP_YOUR_MEMBERSHIP}></Header>
				<Main>
					<Success message={submittedMessage} />
				</Main>
			</>
		);
	}
